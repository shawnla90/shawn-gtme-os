...
export function generateStaticParams() {
  return LEMLIST_WIKI_ENTRIES.map((entry) => ({ slug: entry.id }))
}
...
export async function generateMetadata({
  ...
  const entry = getLemlistWikiEntry(slug)

  if (!entry) {
    return { title: 'Entry Not Found | Lemlist Wiki' }
  }

  const title = `Lemlist Wiki: ${entry.title}`
  const description = entry.description.slice(0, 155)
  const url = `${SITE_URL}/lemlist-wiki/${entry.id}`
...
const lemlistIcon: React.CSSProperties = {
  flexShrink: 0,
  width: 96,
  height: 96,
  display: 'flex',
...
export default async function LemlistWikiEntryPage({
  params,
}: {
  params: Promise<{ slug: string }>
}) {
    ...
    return (
      <div style={pageWrap}>
        <h1 style={terminalHeader}>
          <span style={{ color: 'var(--accent)' }}>$</span> man lemlist-wiki/???
        </h1>
        <p style={{ color: 'var(--text-muted)', fontSize: '14px' }}>
          entry not found. try the{' '}
          <Link href="/lemlist-wiki" style={{ color: 'var(--accent)' }}>
            lemlist wiki
          </Link>
          .
        </p>
  ...
  const catMeta = LEMLIST_WIKI_CATEGORIES.find((c) => c.id === entry.category)

  const relatedEntries = entry.related
    .map((id) => {
      const r = LEMLIST_WIKI_ENTRIES.find((e) => e.id === id)
    ...
    <>
      <BreadcrumbSchema
        items={[
          ...
          { name: entry.title, url: `${SITE_URL}/lemlist-wiki/${entry.id}` },
        ]}
      />
      <script
      ...
      <div style={pageWrap}>
        ...
        {/* Navigation */}
        <div style={navRow}>
          <Link href="/lemlist-wiki" style={navLink}>
            &larr; lemlist wiki
          </Link>
