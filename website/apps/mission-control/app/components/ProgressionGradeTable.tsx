...
export default function ProgressionGradeTable({ dayScoring }: ProgressionGradeTableProps) {
        ...
        <tbody>
          {dayScoring.map((entry) => (
            <tr key={entry.date} className="border-b border-green-900/30 hover:bg-gray-900 transition-colors">
              ...
              <td className="text-right py-1.5 px-2 text-green-300 font-bold">{entry.output_score}</td>
              <td className={`text-center py-1.5 px-2 font-bold ${gradeTextColor(entry.letter_grade)}`}>
                {entry.letter_grade}
              </td>
              <td className="text-right py-1.5 pl-2 text-gray-400">{entry.commits_count}</td>
            </tr>
          ))}
        </tbody>
      </table>
