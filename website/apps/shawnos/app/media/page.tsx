...
export const metadata: Metadata = {
  title: 'Media | Podcasts, Interviews & Appearances',
  description:
    'Podcasts, interviews, panels, and speaking appearances by Shawn Tenam. GTM engineering, AI agents, and building in public.',
  keywords: [
    'shawn tenam podcast',
  ...
  twitter: {
    title: 'Media | Podcasts, Interviews & Appearances',
    description:
      'Podcasts, interviews, panels, and speaking appearances by Shawn Tenam.',
    images: [
...
function categoryLabel(cat: MediaAppearance['category']): { label: string; color: string } {
  switch (cat) {
    case 'podcast':
      return { label: 'PODCAST', color: '#c084fc' }
    case 'interview':
      return { label: 'INTERVIEW', color: '#60a5fa' }
    case 'panel':
      return { label: 'PANEL', color: '#4ade80' }
    case 'speaking':
      return { label: 'SPEAKING', color: '#facc15' }
    case 'article':
      return { label: 'ARTICLE', color: '#fb923c' }
  }
}
  ...
  return (
    <>
      ...
      <PageHero
        compact
        title="Media"
        subtitle="Podcasts, interviews, and appearances"
      />
        ...
        <StatsStagger>
          <StatItem>
            <div style={statBox}>
              ...
              <span style={statLabel}>Appearances</span>
            </div>
          </StatItem>
          <StatItem>
            <div style={statBox}>
              <span style={statNum}>{uniquePlatforms.size}</span>
              <span style={statLabel}>Platforms</span>
            </div>
          </StatItem>
          <StatItem>
            <div style={statBox}>
              <span style={statNum}>{latestDate}</span>
              <span style={statLabel}>Latest</span>
            </div>
          </StatItem>
        </StatsStagger>
      </ScrollRevealSection>
      ...
      {featured && (
        <ScrollRevealSection background="var(--canvas-subtle)">
          ...
          <div style={featuredCard}>
            <div style={typeBadge(categoryLabel(featured.category).color)}>
      ...
      <ScrollRevealSection background="var(--canvas)">
        <div style={sectionTitle}>All Appearances</div>

        <div style={timelineScroll}>
          <div style={timelineTrack}>
            {allAppearances.map((appearance) => {
              const cat = categoryLabel(appearance.category)
              return (
                <div key={appearance.id} style={timelineCard}>
                  <div style={typeBadge(cat.color)}>{cat.label}</div>
      ...
      <ScrollRevealSection background="var(--canvas-subtle)">
        <div style={backlinkNote}>
          <strong style={{ color: 'var(--text-primary)' }}>For podcast hosts and media:</strong>{' '}
          Link back to{' '}
          <a
            href={`${SITE_URL}/media`}
