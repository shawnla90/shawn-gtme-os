...
interface ClassShowcaseGridProps {
...
const classLabel: React.CSSProperties = {
  fontSize: '10px',
    ...
    <div style={{ position: 'relative' }}>
      ...
      {phase === 'locked' && (
        <div
          ...
          role="button"
          tabIndex={0}
          aria-label={`Click to preview locked ${rpgClass} class`}
          onClick={handleClick}
          onKeyDown={(e) => {
...
const EASTER_EGG_CSS = `
  ...
  /* ── Builder: Forge spark — radial amber burst ── */
  .class-flash-builder {
    background: radial-gradient(circle, rgba(245, 158, 11, 0.45) 0%, transparent 70%);
  }
  .class-glow-builder {
    box-shadow: 0 0 12px rgba(245, 158, 11, 0.2), 0 0 4px rgba(245, 158, 11, 0.15);
    animation: glow-builder 2s ease-out;
  }
  ...
  /* ── Scribe: Ink wash — top-down cyan sweep ── */
  .class-flash-scribe {
    background: linear-gradient(180deg, rgba(6, 182, 212, 0.4) 0%, transparent 100%);
  }
  .class-glow-scribe {
    box-shadow: 0 0 12px rgba(6, 182, 212, 0.2), 0 0 4px rgba(6, 182, 212, 0.15);
    animation: glow-scribe 2s ease-out;
  }
  ...
  /* ── Strategist: Lightning — double-flash blue strobe ── */
  .class-flash-strategist {
    background: radial-gradient(circle, rgba(59, 130, 246, 0.5) 0%, transparent 60%);
  }
  .class-glow-strategist {
    box-shadow: 0 0 12px rgba(59, 130, 246, 0.2), 0 0 4px rgba(59, 130, 246, 0.15);
    animation: glow-strategist 1.5s ease-out;
  }
  ...
  /* ── Alchemist: Bubbling flask — rising purple pulse ── */
  .class-flash-alchemist {
    background: radial-gradient(ellipse at bottom, rgba(168, 85, 247, 0.45) 0%, transparent 70%);
  }
  .class-glow-alchemist {
    box-shadow: 0 0 12px rgba(168, 85, 247, 0.2), 0 0 4px rgba(168, 85, 247, 0.15);
    animation: glow-alchemist 3s ease-in-out;
  }
  ...
  /* ── Polymath: Spectrum shimmer — rainbow border cycle ── */
  .class-flash-polymath {
  ...
  .class-glow-polymath {
    animation: glow-polymath 3s linear;
  }
    ...
    <>
      ...
      <div className="class-showcase-grid">
        {ALL_CLASSES.map((rpgClass) => {
          ...
          const isLocked = rpgClass !== currentClass
          ...
          return (
            <div key={rpgClass} style={cardWrapper}>
              ...
              <div style={{ ...classLabel, color: isLocked ? 'var(--text-muted)' : colors(rpgClass) }}>
                {rpgClass}
              </div>
            </div>
          )
...
/* helper: get primary color for a class label */
function colors(rpgClass: RPGClass): string {
  return CLASS_COLORS[rpgClass].primary
}
