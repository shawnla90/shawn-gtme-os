...
function sectionTypeColor(type: WikiSection['type']): string {
  switch (type) {
    case 'prose':
      return ''
    case 'pattern':
...
const SECTION_TYPE_I18N_KEY: Record<string, string> = {
  pattern: 'pattern',
  code: 'code',
    ...
    return (
      <div style={pageWrap}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '14px' }}>
          {t('entryPage.notFoundMessage')}{' '}
          <Link href="/how-to" style={{ color: 'var(--accent)' }}>
            {t('entryPage.notFoundLink')}
          </Link>
          .
        </p>
      </div>
      ...
      <div style={pageWrap}>
        ...
        {entry.sections.length > 0 ? (
          entry.sections.map((section, i) => {
            const color = sectionTypeColor(section.type)
            const i18nKey = SECTION_TYPE_I18N_KEY[section.type]
            return (
              <div key={i} style={sectionWrap}>
                {i18nKey && (
                  <div style={sectionTypeBadge(color)}>
                    {t(`entryPage.sectionTypes.${i18nKey}`)}
                  </div>
                )}
        ...
        {relatedEntries.length > 0 && (
          <>
            <hr style={divider} />
            <div style={relatedLabel}>{t('entryPage.sections.relatedGuides')}</div>
            <div style={relatedWrap}>
        ...
        <div style={navRow}>
          <Link href="/how-to" style={navLink}>
            &larr; {t('entryPage.nav.left')}
          </Link>
          <Link href="/knowledge" style={navLink}>
            {t('entryPage.nav.right')} &rarr;
          </Link>
        </div>
      </div>
    </>
