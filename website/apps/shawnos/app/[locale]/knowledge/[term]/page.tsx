...
export default async function TermPage({
  ...
  const { term } = await params
  const t = SLUG_TO_TERM.get(term)
  const i18n = await getTranslations('Knowledge')
    ...
    return (
      <div style={pageWrap}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '14px' }}>
          {i18n('termPage.notFoundMessage')}{' '}
          <Link href="/knowledge" style={{ color: 'var(--accent)' }}>
            {i18n('termPage.notFoundLink')}
          </Link>
          .
        </p>
      </div>
  ...
  const backLabel =
    t.source === 'gtm' ? i18n('termPage.nav.backToGtm') : i18n('termPage.nav.backToEngineering')
  ...
  return (
    <>
      ...
      <div style={pageWrap}>
        ...
        {/* Why it matters */}
        <div style={sectionLabel}>{i18n('termPage.sections.whyItMatters')}</div>
        <p style={sectionBody}>{t.whyItMatters}</p>

        {/* How you use it */}
        <div style={sectionLabel}>
          {t.source === 'gtm' ? i18n('termPage.sections.howIUseIt') : i18n('termPage.sections.howYouUseIt')}
        </div>
        ...
        {related.length > 0 && (
          <>
            <hr style={divider} />
            <div style={sectionLabel}>{i18n('termPage.sections.relatedTerms')}</div>
        ...
        {['clay', 'claygent', 'sculptor', 'enrichment'].includes(t.slug) && (
          <>
            <hr style={divider} />
            <div style={sectionLabel}>{i18n('termPage.sections.goDeeper')}</div>
            <Link
              href="/clay-wiki"
              style={{
                display: 'inline-flex',
        ...
        <div style={navRow}>
          ...
          <Link href="/knowledge" style={navLink}>
            {i18n('termPage.nav.allTerms')} &rarr;
          </Link>
        </div>
      </div>
    </>
