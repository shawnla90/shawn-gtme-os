  ...
  return (
    <>
        ...
        <StatsStagger>
          <StatItem>
            <div style={statBox}>
              ...
              <span style={statLabel}>{t('stats.recentUpdates')}</span>
            </div>
          </StatItem>
          <StatItem>
            <div style={statBox}>
              <span style={statNum}>{totalFeatures}</span>
              <span style={statLabel}>{t('stats.featuresShipped')}</span>
            </div>
          </StatItem>
          <StatItem>
            <div style={statBox}>
              <span style={statNum}>{categories.length}</span>
              <span style={statLabel}>{t('stats.categories')}</span>
            </div>
          </StatItem>
        </StatsStagger>
      ...
      <ScrollRevealSection background="var(--canvas-subtle)">
        <div style={sectionTitle}>{t('sections.featuresAndLaunches')}</div>

        <div style={timelineScroll}>
          <div style={timelineTrack}>
            {FEATURE_MILESTONES.map((m, i) => {
              const ft = featureTypeStyle(m.type)
              const featureTypeKey = m.type === 'infrastructure' ? 'infrastructure' : m.type
              const inner = (
                <>
                  <div style={typeBadge(ft.color)}>{t(`featureTypes.${featureTypeKey}`)}</div>
      ...
      <ScrollRevealSection background="var(--canvas)">
        <div style={sectionTitle}>{t('sections.allContent')}</div>
